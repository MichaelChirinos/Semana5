...
def insert():
    ...
    if not fullname or not email or not phone:
        return jsonify({'error': 'fullname, email, and phone are required'}), 400
    ...
    result["msg"] = "Se agrego el contacto sin inconvenientes"
...
def update():
    ...
    if not id or not fullname or not email or not phone:
        return jsonify({'error': 'id, fullname, email, and phone are required'}), 400

    contacto = Contact.query.get(id)

    if contacto is None:
        # Return a 404 Not Found status code with an error message
        return jsonify({"error": f"Contact with id {id} not found"}), 404
    ...
    result["msg"] = "Se modifico el contacto sin inconvenientes"
...
def delete():
    ...
    result["data"] = {}
    result["status_code"] = 200
    result["msg"] = "Se elimino el contacto sin inconvenientes"

    # Return a 204 No Content status code with an empty response body
    return jsonify(result), 200
